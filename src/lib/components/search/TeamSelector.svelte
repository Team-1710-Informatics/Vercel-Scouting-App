...
{#if MATCH_KEY_REGEX.test(match.key)}
    <div
        transition:slide
        class="grid grid-cols-1 gap-0 w-fit"
        class:opacity-50={disabled}
    >
        {#each ['red', 'blue'] as all}
            <div
                class={`grid grid-cols-3 w-fit p-2 pb-3 gap-1 ${all === 'red' ? 'bg-red-500 rounded-t-lg' : 'bg-blue-500 rounded-b-lg'}`}
            >
                {#each match.alliances[all].team_keys as t}
                    <div class="relative w-14 h-8">
                        <button
                            ...
                            style={+t.substring(3) == team && all == alliance
                                ? 'top:4px'
