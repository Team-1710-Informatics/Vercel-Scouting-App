...
export const handle = async function ({ event, resolve }) {
    ...
    const user = JSON.parse(
        JSON.stringify(await User.findOne({ token: token }))
    )
...
function currComp(res: any) {
    ...
    res.forEach((e: any) => {
        if (
            timestamp(e.start_date, e.timezone) <= now &&
            timestamp(e.end_date, e.timezone) + 86400000 > now
        )
            curr = e
    })
