...
export const actions = {
    async purchase({ request, locals }){
        ...
        await credi.transaction(user.username, -item.price, `Purchased ${item.name}`);
...
async function getInfo(user:string){
    const items:Merchandise[] = JSON.parse(JSON.stringify(await StoreItem.find()));
    const receipts:Receipt[] = JSON.parse(JSON.stringify(await Receipt.find({user})));
