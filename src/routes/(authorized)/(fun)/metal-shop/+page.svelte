<script>
    ...
    async function load() {
        const c = await (
            await fetch(`${PUBLIC_HOST}/internal/credits/${data.user}`)
        ).json()
...
<middle class="py-10">
    ...
    <p class="mb-5 text-2xl text-teal-400 font-bold">
        <Credits class="mb-5 text-3xl">{Math.trunc($credits)}</Credits> credits
    </p>
    ...
    <div class="grid grid-cols-2 gap-x-1">
        {#each metals as metal}
            ...
            >
                <input hidden value={metal.name} name="type" /><input
                    hidden
                    value={metal.cost}
                    name="cost"
                />
                <button
                    class="font-bold bg-gradient-to-t from-slate-800 to-emerald-800 p-3 w-40 rounded-lg"
                    disabled={$credits < metal.cost || loading}
                    >Buy 1 {metal.name}<br />for {metal.cost} credits</button
                >
            </form>
            {#if metal.name != 'bismuth'}
                ...
                >
                    ...
                    <button
                        class="font-bold bg-gradient-to-t from-slate-800 to-emerald-800 p-3 w-40 rounded-lg"
                        disabled={metal?.upgradeAble}
                        >Buy 1 {metal.name}<br />for 4 {metal.upgrade}</button
                    >
                </form>
            {:else}
                <div />
    ...
    <p class="mb-5 text-2xl text-teal-400 font-bold">
        <span class="mb-5 text-3xl">{data.bismuth}</span> bismuth
    </p>
    <p class="mb-5 text-2xl text-teal-400 font-bold">
        <span class="mb-5 text-3xl">{data.tungsten}</span> tungsten
    </p>
    <p class="mb-5 text-2xl text-teal-400 font-bold">
        <span class="mb-5 text-3xl">{data.tourmaline}</span> tourmaline
    </p>
    <p class="mb-5 text-2xl text-teal-400 font-bold">
        <span class="mb-5 text-3xl">{data.welmanite}</span> welmanite
    </p>
