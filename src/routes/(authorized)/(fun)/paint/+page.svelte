...
<div class="flex flex-col overflow-x-hidden">
    <h1 class="text-5xl mx-auto underline my-5">
        <ColorSwappingText text={'SCOUTING PAINT'} />
    </h1>
    {#if !started}
        <button
            class="w-fit mx-auto my-4 text-3xl border-2 rounded border-black bg-slate-600"
            on:click={() => {
                started = true
                clear()
    ...
    <canvas
        width={innerWidth - 50}
        height="400"
        class="mx-auto border-2 border-black touch-none"
        bind:this={canvas}
        on:touchstart={() => {
            active = true
    ...
    <div class="grid grid-cols-5 mx-auto rounded my-2">
        {#each sizes as size}
            <button
                class="border-2 rounded bg-white mx-1 mt-1"
                on:click={() => {
                    width = size
                }}
            >
                <div
                    class="bg-black m-auto p-0"
    ...
    <button class="w-fit rounded border-2 mx-auto my-2" on:click={clear}
        >Clear/Reset</button
    >
    <button class="w-fit rounded border-2 mx-auto my-2" on:click={download}
        >Download</button
    >
    <a bind:this={link} />
</div>
