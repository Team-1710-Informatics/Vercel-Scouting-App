...
async function payout(t, tickets) {
    ...
    for (let i = 0; i < tickets.length; i++) {
        ...
        if (
            Math.trunc(tickets[i].timestamp / 1000) > res.actual_time &&
            res.actual_time != null
        )
            continue

        sums[tickets[i].alliance] +=
            tickets[i].amount + Math.sqrt(tickets[i].amount) / 0.2
    ...
    return [
        Math.trunc(portion * pot),
        Math.ceil((sums[t.alliance] / pot) * 100),
        Math.round(((t.amount + Math.sqrt(t.amount) / 0.2) / pot) * 100),
    ]
}

async function results(key) {
    const results = await fetch(
        `https://www.thebluealliance.com/api/v3/match/${key}/simple`,
        {
            headers: {
