...
>
    <center class="flex flex-col">
        ...
        {:then credits}
            ...
            >
                <div class="w-fit text-2xl self-center text-teal-500">
                    <Credits class="text-3xl">{credits - wager}</Credits> credits
                </div>
                ...
                <div
                    class="grid grid-cols-2 my-2 -mx-2 bg-gradient-to-br border-y-2
                 {alliance === 'blue'
                        ? 'from-blue-600 via-transparent to-red-600/50'
                        : 'from-blue-600/50 via-transparent to-red-600'}"
                >
                    <label
                        class="accent-blue-500 {alliance === 'blue'
                            ? 'opacity-100 underline decoration-2'
                            : 'opacity-50'}"
                        ><input
                            hidden
                            bind:group={alliance}
                    ...
                    >
                    <label
                        class="accent-red-500 {alliance === 'red'
                            ? 'opacity-100 underline decoration-2'
                            : 'opacity-50'}"
                        ><input
                            hidden
                ...
                <button
                    ...
                    disabled={wager > max(credits) ||
                        wager <= 0 ||
                        wager != Math.trunc(wager) ||
                        loading}>Bet <Credits>{wager}</Credits> credits</button
                >
            </form>
        {/await}
    </center>
