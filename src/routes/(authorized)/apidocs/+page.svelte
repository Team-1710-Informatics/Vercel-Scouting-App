...
<center>
<div>
    <span style="font-size:xx-large;"><u><strong>Scouting API</strong></u></span>
    <p style=""class="box max-w-screen-md box-extension">
        The team 1710 scouting API is a web-based API that allows access to viewing of scouting data. 
        You can integrate scouting information into your own applications via HTTP request.
    </p>
    <br>
    <h1>Parameters</h1>
    <p style="" class="box max-w-screen-md box-extension">
        With each HTTP request for scouting data, the parameters and result is explained below. 
    ...
    {#each eventRoutes as route}
        <button on:click={()=>{route.checked=!route?.checked}} class="event-route">
            <img src={arrow} style={route?.checked?"":"transform:rotate(-90deg);"} alt="">
            {route.url}
        </button>
    {#if route?.checked}
    ...
    {#each teamRoutes as route}
        <button on:click={()=>{route.checked=!route?.checked}} class="team-route">
            <img src={arrow} style={route?.checked?"":"transform:rotate(-90deg);"} alt=""> 
            {route.url}
        </button>
        {#if route?.checked}
    ...
    {#each keys as route}
        <button on:click={()=>{route.checked=!route?.checked}} class="key-route">
            <img src={arrow} style={route?.checked?"":"transform:rotate(-90deg);"} alt=""> 
            {route.url}
        </button>
        {#if route?.checked}
