<script>
    ...
    function nextSmallest(field) {
        let values = []
        buttons.forEach((e) => values.push(e[field]))
        let x = 0
...
<middle>
        ...
        {#if selected != ''}
            {#each buttons as button}
                {#if button.id == selected}
                    ...
                    <label>
                        <input bind:checked={button.disabled} type="checkbox" />
                        Disabled
                    </label><br />
                    <label>
                        <input bind:checked={button.team} type="checkbox" /> Team
                        1710
                    </label><br />
                    <button
                        on:click={deleteButton(button.id)}
                        class="my-0 border-rose-800 border-2 font-bold bg-gradient-to-br from-rose-800 to-slate-800 rounded-lg hover:bg-gradient-to-tl"
                    >
                        Delete
                    </button>
                {/if}
    ...
    <button
        on:click={newButton}
        class="my-10 border-sky-800 border-2 font-bold bg-gradient-to-br from-sky-800 to-slate-800 rounded-lg hover:bg-gradient-to-tl"
    >
        Add a Button
    </button>
</middle>
